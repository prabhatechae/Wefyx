...
public class JwtService {
    ...
    public String validate(String token){try{String[] parts=token.split("\\.");if(parts.length!=3)return null;String content=parts[0]+"."+parts[1];if(!java.security.MessageDigest.isEqual(sign(content),Base64.getUrlDecoder().decode(parts[2])))return null;Map<?,?> claims=json.readValue(Base64.getUrlDecoder().decode(parts[1]),Map.class);if(((Number)claims.get("exp")).longValue()<Instant.now().getEpochSecond())return null;return String.valueOf(claims.get("sub"));}catch(Exception e){return null;}}
